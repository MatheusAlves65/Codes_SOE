    ...
    while (1)
    {
        ...
        for (index = 0; index < count; index++)
        {
            ...
            if (message.text)
            {
                ...
                {
                    char str[4096];
                    if (strstr(message.text, "/iniciar"))
                    {
                        snprintf(str, SIZE_OF_ARRAY(str), "Hello %s", message.from->first_name);
                    }
                    //if (strstr(message.text, "/aciona"))
                    //{
                        //snprintf(str, SIZE_OF_ARRAY(str), "Acionando controle para %s", message.from->first_name);
                        //digitalWrite(controlePin, HIGH);
                        //delay(1000);
                        //digitalWrite(controlePin, LOW);
                    //}
                    //if (strstr(message.text, "/sensor"))
                    //{
                        //for(int i = 0; i < 10; i++){
                            //if(digitalRead(sensorPin)){
                                //snprintf(str, SIZE_OF_ARRAY(str), "Sensor ativo");
                            //}else{
                                //snprintf(str, SIZE_OF_ARRAY(str), "Sensor inativo");
                            //}
                            //delay(250);
                        //}
                    //}
